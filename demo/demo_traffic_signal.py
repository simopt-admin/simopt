...
def main():
    # specifiy what parts you would like to do
    run_design = False #change to True to run ruby and create design over model/problem factors. If false, will run one version of traffic light with using fixed factors
    ...
    pickle_file = "../experiments/outputs/10_min_traffic_group_ASTRODF_on_TRAFFICCONTROL-1.pickle"
    ...
    solver_names = ["ASTRODF"] # can change this to any solver name, make sure to keep formatted as list
    #OPTIONAL: change experiment name to prevent override of previous runs
    experiment_name = "Traffic_Light_10_min"

    # OPTIONAL: Provide additional overrides for problem default factors. If creating a design, any factors used in design will be ignored here.
    ...
    if run_design: # this part will only run if create_design is True
        # Specify the names and min/max/decimal values of the problem factors that will be varied. To vary more factors, add another nested dictonary
        # to design_factor_settings with the min, max, and decimal value that you want to vary. Factor names and data types must exactly match those used in model
        # for any integer factors, dec must be 0
        design_factor_settings = {"runtime": {"min": 60, "max": 7200, "dec": 0}} 
        # Specify the number stacks to use for ruby design creation. More stacks mean more design points. 
        n_stacks = 1
        
        # !! Don't change anthing below this point !!
        
        # create the factor setting file
        with open(f"./data_farming_experiments/{experiment_name}_factor_settings.txt", "w") as file:
            file.write("")
            for factor in design_factor_settings:
                ...
                file.write(data)
        design_factors = design_factor_settings.keys()
        
        # remove design factors from fixed factors if present
        for factor in design_factors:
            if factor in problem_fixed_factors:
                del problem_fixed_factors[factor]
        
        # Create DataFarmingExperiment object for model design
        design_list = create_design(name=problem_name,
                        ...
                        class_type = "problem",
                        csv_filename = f"./data_farming_experiments/{experiment_name}_design.csv"
                        )
