...
results_filename = './4-24_run/4-24_200postreps_results.csv' # file location of csv results for cross design of solver design & problem design
...
for prob_dp in range(prob_n_dp): # cycle through problem versions
    ...
    pd.to_numeric(prob_df['solver_dp'])
    ...
    for sol_dp in range(sol_n_dp):           
        sol_df = prob_df.loc[(prob_df['solver_dp']==sol_dp) | (prob_df['solver_dp']==str(sol_dp)) , ['solver_dp','Final Relative Optimality Gap']]
        pd.to_numeric(sol_df['solver_dp'])
        ...
        avg_gaps.at[sol_dp, 'gamma_1'] = prob_df.loc[prob_df['solver_dp'] == sol_dp, 'gamma_1'].max()
        avg_gaps.at[sol_dp, 'gamma_2'] = prob_df.loc[prob_df['solver_dp'] == sol_dp, 'gamma_2'].max()
        avg_gaps.at[sol_dp, 'eta_1'] = prob_df.loc[prob_df['solver_dp'] == sol_dp, 'eta_1'].max()
        avg_gaps.at[sol_dp, 'eta_2'] = prob_df.loc[prob_df['solver_dp'] == sol_dp, 'eta_2'].max()
    # get default avg gap
...
#     best_solver = avg_gaps.loc[avg_gaps['Average Optimality Gap'] == best_gap, 'solver_dp'].values[0]
