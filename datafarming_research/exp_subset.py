...
def main():
    ...
    for row in range(n_dp):
        ...
        if row in solver_subset:
    
            solver_factors.append(dp_factors)
            solver_names.append(solver_name)
            solver_renames.append(f"{solver_name}_{row}a")
    ...
    sales_price = [6,12,10] # sales price per product unit
    ...
    for index, row in df.iterrows():
        ...
        dp_factors["sales_price"] = [6,12,10]
        ...
        if index in problem_subset:
    
            problem_factors.append(dp_factors)
            problem_names.append(problem_name)
            problem_renames.append(f"{problem_name}_{index}a")
