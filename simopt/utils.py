...
def print_table(name: str, headers: list[str], data: list[tuple] | dict) -> None:
    ...
    data_widths = [
        max(len(str(item)) for item in col) for col in zip(*data, strict=True)
    ]
    header_widths = [len(header) for header in headers]
    max_widths = [
        max(header_width, col_width)
        for header_width, col_width in zip(header_widths, data_widths, strict=True)
    ]
    ...
    header_row = f" {pipe} ".join(
        f"{header:<{width}}" for header, width in zip(headers, max_widths, strict=True)
    )
    ...
    for row in data:
        row_str = f" {pipe} ".join(
            ...
            for item, width in zip(row, max_widths, strict=True)
        )
        rows.append(row_str)
