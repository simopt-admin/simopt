...
class STRONG(Solver):
    ...
    def check_eta_1(self):
        return (
            self.factors["eta_1"] < 1 and self.factors["eta_1"] > self.factors["eta_0"]
        )
        ...
        while expended_budget < problem.factors["budget"]:
            ...
            if delta_T > delta_threshold:
                ...
                grad, Hessian = self.finite_diff(
                    new_solution, BdsCheck, 1, problem, n_r
                )
            ...
            # When trust region size is very small, use the quadratic design.
                ...
                grad, Hessian = self.finite_diff(
                    new_solution, BdsCheck, 2, problem, n_r
                )
                expended_budget += NumOfEval * n_r
                # A while loop to prevent zero gradient
                while norm(grad) == 0:
                ...
                if (rho < eta_0) | ((g_old - g_new) <= 0) | ((r_old - r_new) <= 0):
                    ...
                    while np.sum(result_x != new_x) == 0:
                        ...
                        G, H = self.finite_diff(
                            new_solution, BdsCheck, 2, problem, (sub_counter + 1) * n_r
                        )
                        ...
                        problem.simulate(
                            try_solution,
                            int(n_r + np.ceil(sub_counter ** self.factors["lambda_2"])),
                        )
                        expended_budget += int(
                            n_r + np.ceil(sub_counter ** self.factors["lambda_2"])
                        )
                        ...
                        problem.simulate(
                            ...
                            int(
                                ...
                                - np.ceil((sub_counter - 1) ** self.factors["lambda_2"])
                            ),
                        )
                        expended_budget += int(
                            np.ceil(sub_counter ** self.factors["lambda_2"])
    ...
    def cauchy_point(self, grad, Hessian, new_x, problem):
        ...
        if np.dot(np.matmul(grad, Hessian), grad) <= 0:
            ...
            tau = min(
                1, norm(grad) ** 3 / (delta_T * np.dot(np.matmul(grad, Hessian), grad))
            )
    ...
    def finite_diff(self, new_solution, BdsCheck, stage, problem, n_r):
        ...
        if stage == 2:
            ...
            for i in range(problem.dim):
                if BdsCheck[i] == 0:
                    Hessian[i, i] = (FnPlusMinus[i, 0] - 2 * fn + FnPlusMinus[i, 1]) / (
                        FnPlusMinus[i, 2] ** 2
                    )
                elif BdsCheck[i] == 1:
                    x3 = list(new_x)
                ...
                for j in range(i + 1, problem.dim):
                    ...
                    if BdsCheck[i] ** 2 + BdsCheck[j] ** 2 == 0:
                        ...
                        Hessian[i, j] = (
                            ...
                            - FnPlusMinus[j, 1]
                            + fn6
                        ) / (2 * FnPlusMinus[i, 2] * FnPlusMinus[j, 2])
                    ...
                    # When x on boundary, y not.
                        ...
                        # Compute second order gradient.
                        Hessian[i, j] = (
                            fn5 - FnPlusMinus[j, 0] - fn6 + FnPlusMinus[j, 1]
                        ) / (2 * FnPlusMinus[i, 2] * FnPlusMinus[j, 2] * BdsCheck[i])
                        Hessian[j, i] = Hessian[i, j]
                    # When y on boundary, x not.
                        ...
                        # Compute second order gradient.
                        Hessian[i, j] = (
                            fn5 - FnPlusMinus[i, 0] - fn6 + FnPlusMinus[i, 1]
                        ) / (2 * FnPlusMinus[i, 2] * FnPlusMinus[j, 2] * BdsCheck[j])
                        Hessian[j, i] = Hessian[i, j]
                    elif BdsCheck[i] == 1:
                        if BdsCheck[j] == 1:
                            # Represent f(x+h,y+k).
                            ...
                            # Compute second order gradient.
                            Hessian[i, j] = (
                                fn5 - FnPlusMinus[i, 0] - FnPlusMinus[j, 0] + fn
                            ) / (FnPlusMinus[i, 2] * FnPlusMinus[j, 2])
                            Hessian[j, i] = Hessian[i, j]
                        else:
                            # Represent f(x+h,y-k).
                            x5 = list(new_x)
                            ...
                            # Compute second order gradient.
                            Hessian[i, j] = (
                                FnPlusMinus[i, 0] - fn5 - fn + FnPlusMinus[j, 1]
                            ) / (FnPlusMinus[i, 2] * FnPlusMinus[j, 2])
                            Hessian[j, i] = Hessian[i, j]
                    elif BdsCheck[i] == -1:
                        if BdsCheck[j] == 1:
                            ...
                            # Compute second order gradient.
                            Hessian[i, j] = (
                                FnPlusMinus[j, 0] - fn - fn5 + FnPlusMinus[i, 1]
                            ) / (FnPlusMinus[i, 2] * FnPlusMinus[j, 2])
                            Hessian[j, i] = Hessian[i, j]
                        else:
                            # Represent f(x-h,y-k).
                            ...
                            # Compute second order gradient.
                            Hessian[i, j] = (
                                fn - FnPlusMinus[j, 1] - FnPlusMinus[i, 1] + fn5
                            ) / (FnPlusMinus[i, 2] * FnPlusMinus[j, 2])
                            Hessian[j, i] = Hessian[i, j]
        return grad, Hessian
