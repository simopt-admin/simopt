...
-------
CoordinateSearch
The sequence of solutions generated by the algorithm converges to the set of local optimal solutions with probability 1 and the
estimated optimal values satisfy a central limit theorem. The algorithm starts with an initial solution x0 and sets xˆ∗0 = x0, where xˆ∗k denotes the sample best solution at the end of iteration k. In
the first iteration, the algorithm conducts a line search along x1 directions and finds xˆ∗ 1 which is sample local minimizer along the x1 direction. In the second iteration it
conducts a line search along x2 directions and finds xˆ∗2 and so on so forth. After finishing all d coordinate directions it goes back to the x1 direction.
"""
