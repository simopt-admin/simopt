...
class SSCont(Model):
    ...
    def __init__(self, fixed_factors={}):
        ...
        self.response_names = ['avg_backorder_costs', 'avg_order_costs', 'avg_holding_costs', 'on_time_rate', 'order_rate',
                     'stockout_rate', 'avg_stockout', 'avg_order']
        self.specifications = {
            "demand_mean": {
    ...
    def _replicate(self, diff_factors, rng_list, response_names):
        ...
        Arguments
        ---------
        diff_factors : autograd.numpy.Array
            all factor values which are differentiable
        
        rng_list : [list]  [rng.mrg32k3a.MRG32k3a]
            rngs for model to use when simulating a replication
            
        response_inds : [list] [int]
            indices of responses to return where indices correspond to self.differentiable_factor_names
        ...
        demands = [demand_rng.expovariate(1/factors['demand_mean']) for _ in range(factors["n_days"] + factors["warmup"])]
        # Initialize starting and ending inventories for each period.
        start_inv = [0]*(factors["n_days"] + factors["warmup"])
        start_inv[0] = factors['s']  # Start with s units at period 0.
        end_inv = [0]*(factors["n_days"] + factors["warmup"])
        ...
        orders_received = [0]*(factors["n_days"] + factors["warmup"])
        inv_pos = [0]*(factors["n_days"] + factors["warmup"])
        orders_placed = [0]*(factors["n_days"] + factors["warmup"])
        orders_outstanding = [0]*(factors["n_days"] + factors["warmup"])
        # Run simulation over time horizon.
        for day in range(factors["n_days"] + factors["warmup"]):
            ...
            orders_placed[day] = np.max(((inv_pos[day] < factors['s']) * (factors['S'] - inv_pos[day])), 0)
            if orders_placed[day] > 0:
                lead = lead_rng.poissonvariate(factors['lead_mean'])
                for future_day in range(day + 1, day + lead + 1):
                    if future_day < factors["n_days"] + factors["warmup"]:
                        orders_outstanding[future_day] = orders_outstanding[future_day] + orders_placed[day]
                if day + lead + 1 < factors["n_days"] + factors["warmup"]:
                    orders_received[day + lead + 1] = orders_received[day + lead + 1] + orders_placed[day]
                    # Calculate starting inventory for next period.
            if day < factors["n_days"] + factors["warmup"] - 1:
                start_inv[day + 1] = end_inv[day] + orders_received[day + 1]
        # Calculate responses from simulation data.
        start_inv = np.array(start_inv)
        end_inv = np.array(end_inv)
        orders_received = np.array(orders_received)
        inv_pos = np.array(inv_pos)
        orders_placed = np.array(orders_placed)
        orders_outstanding = np.array(orders_outstanding)
        order_rate = np.mean(orders_placed[factors["warmup"]:] > 0)
        stockout_rate = np.mean(end_inv[factors["warmup"]:] < 0)
        avg_order_costs = np.mean(factors['fixed_cost'] * (orders_placed[factors["warmup"]:] > 0) +
                                  factors['variable_cost'] * orders_placed[factors["warmup"]:])
        avg_holding_costs = np.mean(factors['holding_cost'] * end_inv[factors["warmup"]:] * [end_inv[factors["warmup"]:] > 0])
        on_time_rate = 1 - np.sum(np.min(np.vstack((demands[factors["warmup"]:], demands[factors["warmup"]:] - start_inv[factors["warmup"]:])), axis=0)
                                        * ((demands[factors["warmup"]:] - start_inv[factors["warmup"]:]) > 0))/np.sum(demands[factors["warmup"]:])
        avg_backorder_costs = factors['backorder_cost']*(1 - on_time_rate)*np.sum(demands[factors["warmup"]:])/float(factors["n_days"])
        if np.array(np.where(end_inv[factors["warmup"]:] < 0)).size == 0:
            avg_stockout = 0
        else:
            avg_stockout = -np.mean(end_inv[factors["warmup"]:][np.where(end_inv[factors["warmup"]:] < 0)])
        if np.array(np.where(orders_placed[factors["warmup"]:] > 0)).size == 0:
            avg_order = 0
        else:
            avg_order = np.mean(orders_placed[factors["warmup"]:][np.where(orders_placed[factors["warmup"]:] > 0)])
