...
class Main_Menu_Window(tk.Tk):
    ...
    def open_new_experiment(self):
        ...
        self.new_experiment_window.geometry("1000x850")
        self.new_experiment_window.title("SimOpt Library Graphical User Interface - New Experiment Window")
        self.new_experiment_app= New_Experiment_Window(self.new_experiment_window)
...
class New_Experiment_Window(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        self.display_problem_list_frame = tk.Frame(master = self.display_sol_prob_list_frame)
        self.display_problem_list_frame.grid(row = 0, column = 1)
        
        
        
        self.solver_list_canvas = tk.Canvas(master = self.display_solver_list_frame)
        self.solver_list_canvas.grid(row = 1, column = 0)
        
        self.problem_list_canvas = tk.Canvas(master = self.display_problem_list_frame)
        self.problem_list_canvas.grid(row = 1, column = 0)
        
        self.solver_list_title = tk.Label(master = self.display_solver_list_frame, text = 'Created Solvers', font = 'Calibri 15 bold')
        ...
        self.run_experiment_button.grid(row = 3, column = 0)
    ...
    def show_factor_defaults(self, base_object, frame, factor_dict = None, IsModel = False, first_row = 0, design_factors=None):
        if design_factors is None: #not a loaded design
            design_factors = []
        ...
        for  index, factor in enumerate(base_object.specifications):
            ...
            factorname_label = tk.Label (master = frame, text = display_name, font = "Calibri 13", width = 80, anchor = 'w')
            factorname_label.grid( row = que_length, column = 0, sticky = tk.N + tk.W)
            
            str_type = self.get_datatype_str(factor_datatype) #get str version of data type
            
            
            frame.grid_rowconfigure(que_length, weight =1)

            # Add label for factor type
            factortype_label = tk.Label (master = frame, text = str_type, font = "Calibri 13", width = 20, anchor = 'w')
            factortype_label.grid( row = que_length, column = 1, sticky = tk.N + tk.W)
            
            if factor in design_factors:
                factor_default = 'N/A'
                widget_state = 'disabled'
            else:
                widget_state = 'normal'
            
            if factor_datatype is bool:
                
                # Add option menu for true/false
                default_value = tk.StringVar() #Variable to store selected bool factor state
                default_value.set('TRUE') #Set default bool option
                bool_menu = ttk.OptionMenu(frame, default_value, factor_default, 'TRUE', 'FALSE')
                bool_menu.grid( row = que_length, column = 2, sticky = tk.N + tk.W)
                bool_menu.configure(state=widget_state)
            ...
            if factor_datatype == list or factor_datatype == tuple:
                ...
                default_entry.configure(state=widget_state)
    ...
    def datafarming_options(self, base_object, frame, IsModel = False, first_row = 0):
    ...
    def initialize_problem_frame(self): 
        ...
        self.problem_factor_display_canvas = tk.Canvas(master = self.problem_frame)
        ...
        self.headerdefault_label = tk.Label(master = self.problem_frame, text = 'Default Value', font = "Calibri 13 bold", width = 20 )
    ...
    def show_problem_factors(self, event = None):
        ...
        # # show model factors and store default widgets and default values to these
        self.model_defaults, new_last_row = self.show_factor_defaults(base_object = self.problem_object, frame = self.problem_factor_display_canvas, IsModel = True, first_row = last_row+1)
        # combine default dictionaries
    ...
    def initialize_solver_frame(self):
        #clear previous selections
    ...
    def show_problem_datafarm(self, event = None):
        ...
        # show model factors and store default widgets and default values to these
        self.model_datafarm_defaults, new_last_row = self.show_factor_defaults(base_object = self.problem_datafarm_object, frame = self.problem_factor_display_canvas, IsModel = True, first_row = last_row+1)
        self.model_checkstates, self.model_min_vals, self.model_max_vals, self.model_dec_vals, self.model_datafarm_widgets, new_last_row = self.show_datafarming_options(self.problem_datafarm_object, self.problem_factor_display_canvas, True, last_row+1)
        ...
        problem_name = self.get_unique_name(self.master_problem_dict, f"{self.problem_datafarm_object.name}")
    ...
    def show_solver_datafarm(self, event = None):
        ...
        solver_name = self.get_unique_name(self.master_solver_dict, f"{self.solver_datafarm_object.name}")
    ...
    def create_solver_design(self):
        ...
        with open(f"./data_farming_experiments/{self.solver_design_name}_factor_settings.txt", "w") as settings_file:
            settings_file.write("")    
        for factor in self.solver_design_factors:
            ...
            with open(f"./data_farming_experiments/{self.solver_design_name}_factor_settings.txt", "a") as settings_file:
        ...
        self.solver_design_list = create_design( name = self.solver_datafarm_object.name,
                                                ...
                                                factor_settings_filename = f'{self.solver_design_name}_factor_settings',
                                                fixed_factors = self.solver_fixed_factors,
                                                cross_design_factors = self.solver_cross_design_factors,
                                                n_stacks = n_stacks,
                                                design_type = design_type,
                                                csv_filename= f'./data_farming_experiments/{self.solver_design_name}.csv'
                                                )
        # display design tree
        self.display_design_tree(f"./data_farming_experiments/{self.solver_design_name}.csv", self.solver_datafarm_frame, row = 5)
    ...
    def create_problem_design(self):
        ...
        ''' Create factor settings txt file'''
        with open(f"./data_farming_experiments/{self.problem_design_name}_factor_settings.txt", "w") as settings_file:
            settings_file.write("")    
        for factor in self.problem_design_factors:
            ...
            if factor_datatype == float:
                ...
                dec_val = '0'
            data_insert = f"{min_val} {max_val} {dec_val}\n"
            with open(f"./data_farming_experiments/{self.problem_design_name}_factor_settings.txt", "a") as settings_file:
                settings_file.write(data_insert)
    
                
        self.problem_design_list = create_design( name = self.problem_datafarm_object.name,
                                                factor_headers = self.problem_design_factors,
                                                factor_settings_filename = f'{self.problem_design_name}_factor_settings',
                                                fixed_factors = self.problem_fixed_factors,
                                                cross_design_factors = self.problem_cross_design_factors,
                                                n_stacks = n_stacks,
                                                design_type = design_type,
                                                IsProblem = True,
                                                csv_filename=f'./data_farming_experiments/{self.problem_design_name}.csv')
        ...
        self.display_design_tree(f"./data_farming_experiments/{self.problem_design_name}.csv", self.design_display_frame, row = 1)
    ...
    def convert_proper_datatype(self, fixed_factors, base_object, var=True):
    ...
    def open_post_processing_window(self, experiment_name):
        ...
        self.post_processing_window.title("Simopt Graphical User Interface - Experiment Options")
        ...
        # Title label
        self.title_label = tk.Label(master = self.main_frame, text = f'Options for {experiment_name}.', font = 'Calibri 15 bold')
        self.title_label.grid(row = 0, column = 0, sticky = 'nsew')
    ...
    def open_plotting_window(self):
        ...
        self.subtitle_label = tk.Label(master = self.title_frame, text = subtitle, font = 'Calibri 13')
        ...
        self.experiment_selection_label = tk.Label(master = self.plot_selection_frame, text = 'Select Experiment', font = 'Calibri 13')
        ...
        plot_types = ["All Progress Curves", "Mean Progress Curve", "Quantile Progress Curve", "Solve time CDF", "Area Scatter Plot", "CDF Solvability","Quantile Solvability","CDF Difference Plot", "Quantile Difference Plot", "Terminal Progress Plot", "Terminal Scatter Plot"]

        # plot options
        self.plot_options_frame.grid(row=1, column=1)
        self.plot_type_label = tk.Label(master = self.plot_options_frame, text = 'Select Plot Type', font = 'Calibri 13')
        self.plot_type_label.grid(row=0,column=0)
        self.plot_type_var = tk.StringVar()
        plot_types = ['Solvability CDF', 'Solvability Profile', 'Area Scatter Plot', 'Progress Curve', 'Terminal Progress', 'Terminal Scatter Plot']
        self.plot_type_menu = ttk.OptionMenu(self.plot_options_frame, self.plot_type_var, 'Plot Type', *plot_types, command= self.show_plot_options)
        ...
        self.workspace_label = tk.Label(master=self.plotting_workspace_frame, text='Created Plots by Experiment', font = 'Calibri 15' )
    ...
    def update_plot_menu(self, experiment_name):
        ...
        if len(solver_factor_set) == len(self.plot_experiment.solvers[0].factors): # if set length is the same as the fist solver in the experiment
            self.all_same_solver = True
        else:
            self.all_same_solver = False
        
        # determine if all problems in experiment have the same factor options
        n_prob_factors = len(self.plot_experiment.problems[0].factors) + len(self.plot_experiment.problems[0].model.factors)
        if len(problem_factor_set) == n_prob_factors: # if set length is the same as the fist problem in the experiment
        ...
        if self.all_same_solver: 
            ...
            self.solver_tree.heading('Solver Name', text='Solver Name') # set heading for name column
            for factor in self.plot_experiment.solvers[0].factors.keys():
                self.solver_tree.heading(factor, text=factor) #set column header text to factor names
            ...
            self.solver_tree['columns'] = ['Solver Name'] # set columns just to solver name
            self.solver_tree.heading('Solver Name', text='Solver Name')
            for index, solver in enumerate(self.plot_experiment.solvers):
                self.solver_tree.insert("", index, text=index, values=[solver.name])
        ...
        if self.all_same_problem: 
            ...
            self.problem_tree.heading('Problem Name', text='Problem Name') # set heading for name column
            for factor in factors:
                self.problem_tree.heading(factor, text=factor) #set column header text to factor names
            ...
            self.problem_tree.heading('Problem Name', text='Problem Name') # set heading for name column
            for index, problem in enumerate(self.plot_experiment.problems):
                self.problem_tree.insert("", index, text=index, values=[problem.name])
    ...
    def show_plot_options(self, plot_type):
        ...
        self.all_label = tk.Label(master= self.more_options_frame, text = 'Plot all solvers together?', font = 'Calibri 13')
        self.all_label.grid(row=1,column=0)
        self.all_var = tk.StringVar()
        self.all_var.set('Yes')
        self.all_menu = ttk.OptionMenu(self.more_options_frame, self.all_var, 'Yes', 'Yes','No', command=self.disable_legend)
        ...
        if plot_type == 'Progress Curve':
            ...
            self.plot_description = tk.Label(master= self.more_options_frame, text = description, font = 'Calibri 13')
            self.plot_description.grid(row=0,column=0, columnspan=2)
            
            # select subplot type
            self.subplot_type_label = tk.Label(master= self.more_options_frame, text = 'Type', font = 'Calibri 13')
            ...
            self.subplot_type_menu = ttk.OptionMenu(self.more_options_frame, self.subplot_type_var, 'all', *subplot_type_options, command=self.enable_ref_solver)
            self.subplot_type_menu.grid(row=2, column = 1)
            
            #beta entry
            self.beta_label = tk.Label(master= self.more_options_frame, text = 'Quantile Probability (0.0-1.0)', font = 'Calibri 13')
            self.beta_label.grid(row=4, column =0)
            self.beta_var = tk.StringVar()
            self.beta_var.set('0.5') #default value
            self.beta_entry = tk.Entry(master = self.more_options_frame, textvariable=self.beta_var)
            self.beta_entry.grid(row=4, column=1)
            self.beta_entry.configure(state='disabled')
            
            #normalize entry
            self.normalize_label = tk.Label(master= self.more_options_frame, text = 'Normalize Optimality Gaps?', font = 'Calibri 13')
            self.normalize_label.grid(row=3, column=0)
            self.normalize_var = tk.StringVar()
            self.normalize_var.set('Yes')
            self.normalize_menu = ttk.OptionMenu(self.more_options_frame, self.normalize_var, 'Yes', 'Yes','No')
            self.normalize_menu.grid(row=3, column=1)
   
            
            # num bootstraps entry
            self.boot_label = tk.Label(master= self.more_options_frame, text = 'Number Bootstrap Samples', font = 'Calibri 13')
            self.boot_label.grid(row=5, column =0)
            self.boot_var = tk.StringVar()
            self.boot_var.set('100') #default value
            self.boot_entry = tk.Entry(master = self.more_options_frame, textvariable=self.boot_var)
            self.boot_entry.grid(row=5, column=1)
            self.boot_entry.configure(state='disabled')
            
            # confidence level entry
            self.con_level_label = tk.Label(master= self.more_options_frame, text = 'Confidence Level (0.0-1.0)', font = 'Calibri 13')
            self.con_level_label.grid(row=6, column =0)
            self.con_level_var = tk.StringVar()
            self.con_level_var.set('0.95') #default value
            self.con_level_entry = tk.Entry(master = self.more_options_frame, textvariable=self.con_level_var)
            self.con_level_entry.grid(row=6, column=1)
            self.con_level_entry.configure(state='disabled')
            
            # plot CIs entry
            self.plot_CI_label = tk.Label(master= self.more_options_frame, text = 'Plot Confidence Intervals?', font = 'Calibri 13')
            self.plot_CI_label.grid(row=7,column=0)
            self.plot_CI_var = tk.StringVar()
            self.plot_CI_var.set('Yes')
            self.plot_CI_menu = ttk.OptionMenu(self.more_options_frame, self.plot_CI_var, 'Yes', 'Yes','No')
            self.plot_CI_menu.grid(row=7, column=1)
            self.plot_CI_menu.configure(state='disabled')
            
            # plot max HW entry
            self.plot_hw_label = tk.Label(master= self.more_options_frame, text = 'Plot Max Halfwidth?', font = 'Calibri 13')
            self.plot_hw_label.grid(row=8,column=0)
            self.plot_hw_var = tk.StringVar()
            self.plot_hw_var.set('Yes')
            self.plot_hw_menu = ttk.OptionMenu(self.more_options_frame, self.plot_hw_var, 'Yes', 'Yes','No')
            self.plot_hw_menu.grid(row=8, column=1)
            self.plot_hw_menu.configure(state='disabled')
            
            #legend location
            self.legend_label = tk.Label(master= self.more_options_frame, text = 'Legend Location', font = 'Calibri 13')
            self.legend_label.grid(row=9,column=0)
            loc_opt = ['best', 'upper right', 'upper left', 'lower left', 'lower right', 'right', 'center left', 'center right', 'lower center', 'upper center', 'center']
            self.legend_var = tk.StringVar()
            self.legend_var.set('best')
            self.legend_menu = ttk.OptionMenu(self.more_options_frame, self.legend_var, 'best', *loc_opt)
        ...
        if plot_type == 'Solvability CDF':
            ...
            #solve tol entry
            self.solve_tol_label = tk.Label(master= self.more_options_frame, text = 'Solve Tolerance', font = 'Calibri 13')
            self.solve_tol_label.grid(row=2, column=0)
            self.solve_tol_var = tk.StringVar()
            self.solve_tol_var.set(0.1)
            self.solve_tol_entry = tk.Entry(master=self.more_options_frame, textvariable=self.solve_tol_var)
        ...
        if plot_type == 'Solvability Profile':
            ...
            self.subplot_type_label = tk.Label(master= self.more_options_frame, text = 'Type', font = 'Calibri 13')
            self.subplot_type_label.grid(row=2, column=0)
            subplot_type_options = ['CDF Solvability', 'Quantile Solvability', 'Difference of CDF Solvablility', 'Difference of Quantile Solvability']
            self.subplot_type_var = tk.StringVar()
            self.subplot_type_var.set('CDF Solvability')
            self.subplot_type_menu = ttk.OptionMenu(self.more_options_frame, self.subplot_type_var, 'CDF Solvability', *subplot_type_options, command=self.enable_ref_solver)
            self.subplot_type_menu.grid(row=2, column = 1)
        ...
        if plot_type == 'Terminal Progress':
            ...
            self.subplot_type_menu = ttk.OptionMenu(self.more_options_frame, self.subplot_type_var, 'violin', *subplot_type_options)
            self.subplot_type_menu.grid(row=2, column = 1)
        ...
        self.solver_set_label = tk.Label(master=self.more_options_frame, text = 'Solver Group Name to be Used in Title', font='Calibri 13')
        self.solver_set_label.grid(row=new_row,column=0)
        self.solver_set_var = tk.StringVar()
        self.solver_set_var.set("SOLVER_SET")
        self.solver_set_entry = tk.Entry(master=self.more_options_frame, textvariable=self.solver_set_var)
        self.solver_set_entry.grid(row=new_row, column=1)
        self.solver_set_entry.configure(state='normal') #set disabled unless all in is true
        
        if plot_type in ['Terminal Scatter Plot', 'Solvability Profile', 'Area Scatter Plot']:
            self.problem_set_label = tk.Label(master=self.more_options_frame, text = 'Problem Group Name to be Used in Title', font='Calibri 13')
            self.problem_set_label.grid(row=new_row+1,column=0)
            self.problem_set_var = tk.StringVar()
            self.problem_set_var.set("PROBLEM_SET")
            self.problem_set_entry = tk.Entry(master=self.more_options_frame, textvariable=self.problem_set_var)
            self.problem_set_entry.grid(row=new_row+1, column=1)
            self.problem_set_entry.configure(state='normal') #set disabled unlass all in is true
            
        # file extension
        self.ext_label = tk.Label(master= self.more_options_frame, text = 'Save image as:', font = 'Calibri 13')
        self.ext_label.grid(row=new_row+2,column=0)
        self.ext_var = tk.StringVar()
        self.ext_var.set('.png')
        self.ext_menu = ttk.OptionMenu(self.more_options_frame, self.ext_var, '.png', *self.ext_options)
        self.ext_menu.grid(row=new_row+2,column=1)
        
        # plot button
        self.plot_button = ttk.Button(master=self.plot_options_frame, text='Plot', command = self.plot)
        self.plot_button.grid(row=2, column=0)
        
    def disable_legend(self, event=None): # also enables/disables solver & problem group names
        if self.all_var.get() == 'Yes' and self.plot_type != 'Terminal Progress':
            self.legend_menu.configure(state='normal')
            self.solver_set_entry.configure(state='normal')
            if self.plot_type in ['Terminal Scatter Plot', 'Solvability Profile', 'Area Scatter Plot']:
    ...
    def enable_ref_solver(self, plot_type):
        ...
        if plot_type in ['CDF Solvability', 'Quantile Solvability']:
            self.ref_solver_menu.configure(state='disabled')
        elif plot_type in ['Difference of CDF Solvablility', 'Difference of Quantile Solvability']:
            self.ref_solver_menu.configure(state='normal')
            
        #enable beta (also works for progress curves )
        if plot_type in ['Quantile Solvability', 'Difference of Quantile Solvability', 'quantile']:
    ...
    def get_selected_solvers(self, event=None): #upddates solver list and options menu for reference solver when relevant
        ...
        if all_solvers:
            ...
            for item in selected_items:
                ...
                solver = self.plot_experiment.solvers[solver_index] # get corresponding solver from experiment
    ...
    def get_selected_problems(self, event=None):
        ...
        if all_problems:
            ...
            for item in selected_items:
                ...
                problem = self.plot_experiment.problems[problem_index] # get corresponding problem from experiment
                self.selected_problems.append(problem)
                
    def update_ref_solver(self):
        
        saved_solver = self.ref_solver_var.get() # save previously selected reference solver
        if len(self.selected_solvers) != 0:
            solver_options = []
            for solver in self.selected_solvers: # append solver names to options list
                solver_options.append(solver.name)
            if saved_solver not in solver_options: #set new default if previous option was deselected
        ...
        self.ref_solver_menu = ttk.OptionMenu(self.more_options_frame, self.ref_solver_var, saved_solver, *solver_options)
        self.ref_solver_menu.grid(row=9,column=1)
        if self.subplot_type_var.get() in ['CDF Solvability', 'Quantile Solvability']: #disable if not correct plot type
            self.ref_solver_menu.configure(state='disabled')
    ...
    def plot(self):
        ...
        if len(self.selected_problems) == 0 or len(self.selected_solvers) == 0: #show message that no solvers or problems have been selected
            if len(self.selected_problems) == 0 and len(self.selected_solvers) == 0:
                ...
                for problem in self.selected_problems:
                    for solver_group in self.plot_experiment.experiments:
                        for dp in solver_group:
                            if id(dp.solver) == id(solver) and id(dp.problem) == id(problem):
                                solver_list.append(dp)
            ...
            if all_in and self.plot_type != 'Terminal Progress': # only get legend location if all in one is selected
            ...
            if self.plot_type == 'Progress Curve':
                ...
                for i in range(n_problems):
                    ...
                    returned_path = plot_progress_curves(experiments = prob_list, 
            ...
            if self.plot_type == 'Solvability CDF':
                ...
                for i in range(n_problems):
                    ...
                    for solver_group in exp_sublist:
                        prob_list.append(solver_group[i])
                    returned_path = plot_solvability_cdfs(experiments = prob_list,
                                                      solve_tol = solve_tol,
                                                      all_in_one = all_in,
                                                      n_bootstraps = n_boot,
                                                      conf_level = con_level,
...
class Data_Farming_Window():
    ...
    def load_design(self):
        ...
        for  factor_idx, factor in enumerate(self.model_object.specifications):
    ...
    def create_design(self, *args):
        ...
        self.design_filename = f"{self.model_name}_design"
        self.csv_filename = f"./data_farming_experiments/{self.experiment_name}_design.csv"  
