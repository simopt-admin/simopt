...
from simopt.data_farming_base import (
    DataFarmingExperiment,
    DataFarmingMetaExperiment,
)
...
class Experiment_Window(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        self.macro_definition = tk.Label(master=self.master, text="", font="Calibri 13")
        ...
        self.macro_entry = ttk.Entry(
            master=self.master, textvariable=self.macro_var, justify=tk.LEFT, width=10
        )
        ...
        self.pickle_file_pathname_label = tk.Label(
            master=self.master, text="File Selected:", font="Calibri 13"
        )
        ...
        self.vert_scroll_bar = Scrollbar(
            self.queue_label_frame, orient="vertical", command=self.queue_canvas.yview
        )
        self.horiz_scroll_bar = Scrollbar(
            self.queue_label_frame, orient="horizontal", command=self.queue_canvas.xview
        )
        ...
        self.queue_canvas.create_window(
            (0, 0), window=self.queue_frame, anchor="nw", tags="self.queue_frame"
        )
        ...
        for heading in self.heading_list:
            ...
            label = tk.Label(
                master=self.tab_three, text=heading, font="Calibri 14 bold"
            )
            label.grid(row=0, column=self.heading_list.index(heading), padx=10, pady=3)
    ...
    def show_problem_factors(self, *args):
        ...
        self.factor_notebook_problem.add(
            self.factor_tab_one_problem, text=str(self.problem_var.get()) + " Factors"
        )
        ...
        for heading in self.factor_heading_list_problem:
            ...
            label_problem = tk.Label(
                master=self.factor_tab_one_problem, text=heading, font="Calibri 14 bold"
            )
            label_problem.grid(
                row=0,
        ...
        for num, factor_type in enumerate(
            ...
            description = (
                self.problem_object().specifications[factor_type].get("description")
            )
        ...
        for heading in self.factor_heading_list_oracle:
            ...
            label_oracle = tk.Label(
                master=self.factor_tab_one_oracle, text=heading, font="Calibri 14 bold"
            )
            label_oracle.grid(
                row=0,
        ...
        for factor_type in self.oracle_object().specifications:
            ...
            description = (
                self.oracle_object().specifications[factor_type].get("description")
            )
    ...
    def show_solver_factors(self, *args):
        ...
        self.factor_notebook_solver.add(
            self.factor_tab_one_solver, text=str(self.solver_var.get()) + " Factors"
        )
        ...
        for heading in self.factor_heading_list_solver:
            ...
            label = tk.Label(
                master=self.factor_tab_one_solver, text=heading, font="Calibri 14 bold"
            )
            label.grid(
                row=0,
        ...
        self.save_label_solver = tk.Label(
            master=self.factor_tab_one_solver, text="save solver as", font="Calibri 13"
        )
        ...
        for factor_type in self.solver_object().specifications:
            ...
            description = (
                self.solver_object().specifications[factor_type].get("description")
            )
        ...
        if (
            ...
            if int(self.macro_entry.get()) != 0:
                ...
                if compatibility_result == "":
                    ...
                    self.experiment_master_list[place][5][0]["crn_across_solns"] = (
        ...
        for problem_factor in self.problem_factors_list:
            ...
            if index > 0:
                ...
                if keys[index - 1] == "initial_solution" and isinstance(
                    ast.literal_eval(problem_factor.get()), int
                ):
                    t = (ast.literal_eval(problem_factor.get()),)
                    # (t)
                    self.problem_factors_dictionary[keys[index - 1]] = t
                else:
                    self.problem_factors_dictionary[keys[index - 1]] = ast.literal_eval(
                        problem_factor.get()
                    )
    ...
    def plot_meta_function(self, integer):
        ...
        Plot_Window(
            self.postrep_window, self, experiment_list=exps, metaList=self.my_experiment
        )
        ...
        for i, exp in enumerate(newlist):
            if exp.post_norm_ready:
                ...
                self.widget_norm_list.append(
                    [self.problem_added, self.solver_added, self.select_checkbox]
                )
    ...
    def make_meta_experiment_func(self):
        ...
        if response:
            ...
            self.add_meta_exp_to_frame(
                n_macroreps=None, input_meta_experiment=self.meta_experiment_created
            )
    ...
    def view_meta_function(self, row_num):
        ...
        self.problem_label2 = tk.Label(
            master=self.master, text="Group Problem(s):*", font="Calibri 13"
        )
        ...
        self.solver_label2 = tk.Label(
            master=self.master, text="Group Solver(s):*", font="Calibri 13"
        )
    ...
    def show_solver_factors2(self, row_index, *args):
        ...
        self.factor_notebook_solver.add(
            self.factor_tab_one_solver, text=str(self.solver_var2.get()) + " Factors"
        )
        ...
        for factor_type in self.default_solver_object.specifications:
            ...
            datatype = self.default_solver_object.specifications[factor_type].get(
                "datatype"
            )
            description = self.default_solver_object.specifications[factor_type].get(
                "description"
            )
            default = self.default_solver_object.specifications[factor_type].get(
                "default"
            )
    ...
    def show_problem_factors2(self, row_index, *args):
        ...
        self.factor_notebook_problem.add(
            self.factor_tab_one_problem, text=str(self.problem_var2.get()) + " Factors"
        )
        ...
        for num, factor_type in enumerate(
            ...
            datatype = self.default_problem_object.specifications[factor_type].get(
                "datatype"
            )
            description = self.default_problem_object.specifications[factor_type].get(
                "description"
            )
            ...
            if datatype != bool:
                ...
                if datatype == tuple and len(default) == 1:
                    self.int_float_entry_problem.insert(
                        index=tk.END,
                        string=str(self.custom_problem_object.factors[factor_type][0]),
                    )
                else:
                    self.int_float_entry_problem.insert(
                        index=tk.END,
                        string=str(self.custom_problem_object.factors[factor_type]),
                    )
        ...
        for factor_type in self.default_oracle_object.specifications:
            ...
            datatype = self.default_oracle_object.specifications[factor_type].get(
                "datatype"
            )
            description = self.default_oracle_object.specifications[factor_type].get(
                "description"
            )
            default = self.default_oracle_object.specifications[factor_type].get(
                "default"
            )
    ...
    def show_solver_factors_2(self, *args):
        if args and len(args) == 3 and not args[2]:
            pass
        else:
            self.update_problem_list_compatability()
        ...
        if args and len(args) == 3 and args[0] is True:
            oldname = args[1][5][1]
...
class Data_Farming_Window:
    def __init__(self, master, main_widow, forced_creation=False):
        if not forced_creation:
            ...
            self.or_label = tk.Label(
                master=self.modelselect_frame, text="OR", font="Calibri 13", width=20
            )
    ...
    def load_design(self):
        ...
        self.model_object = model_unabbreviated_directory[
            self.selected_model
        ]()  # Eventually allow selection here
        ...
        for factor in self.default_factor_list:
            ...
            self.factorname_label.grid(
                row=self.factor_que_length, column=0, sticky=tk.N + tk.W, padx=10
            )
            ...
            self.default_entry.grid(
                row=self.factor_que_length, column=2, sticky=tk.N + tk.W, columnspan=5
            )
        ...
        self.change_def_button = tk.Button(
            master=self.change_def_frame,
            text="Change Experiment Defaults",
            font="Calibri 13",
            width=30,
            command=self.update_defaults_button,
        )
        ...
        with open(
            "./data_farming_experiments/model_factors_design.txt", "w", encoding="utf-8"
        ) as design_file:
            design_file.write("")
        with open(self.design_csv_name, "r") as design_csv:
            ...
            for row in design_csv:
                ...
                with open(
                    "./data_farming_experiments/model_factors_design.txt",
                    "a",
                    encoding="utf-8",
                ) as design_file:
    ...
    def show_model_factors(self, *args):
        ...
        for factor in self.model_object.specifications:
            ...
            if self.factor_datatype == float:
                ...
                self.min_label = tk.Label(
                    master=self.min_frame, text="Min Value", font="Calibri 13", width=10
                )
                ...
                self.max_label = tk.Label(
                    master=self.max_frame, text="Max Value", font="Calibri 13", width=10
                )
        ...
        self.stack_menu = ttk.OptionMenu(
            self.design_frame,
            self.stack_var,
            "Stacks",
            *self.stack_list,
            command=self.get_design_pts,
        )
    ...
    def display_design_tree(self):
        ...
        with open(self.design_csv_name, "r") as design_file:
            ...
            for row in reader:
                ...
                self.design_tree.insert(
                    "", "end", text=row_index, values=data_insert, tag="style"
                )
                row_index += 1
        ...
        self.rep_label = tk.Label(
            master=self.run_frame, text="Replications", font="Calibri 13", width=20
        )
        ...
        self.crn_label = tk.Label(
            master=self.run_frame, text="CRN", font="Calibri 13", width=20
        )
    ...
    def create_design(self, *args):
        ...
        with open(
            "./data_farming_experiments/model_factors.txt", "w"
        ...
        for factor in self.model_object.specifications:
            ...
            if self.factor_include is True:
                ...
                if self.factor_datatype == float or self.factor_datatype == int:
                    ...
                    self.data_insert = (
                        self.factor_min + " " + self.factor_max + " " + self.factor_dec
                    )
                    with open(
                        "./data_farming_experiments/model_factors.txt", "a"
                    ) as self.model_design_factors:
                        self.model_design_factors.write(self.data_insert + "\n")

            # Include fixed default values in design
            if self.factor_include is False:
        ...
        command = (
            "stack_nolhs.rb -s "
            + num_stack
            + f" ./data_farming_experiments/{factor_settings_filename}.txt > ./data_farming_experiments/{factor_settings_filename}_design.txt"
        )
        ...
        self.design_csv_name = (
            "./data_farming_experiments/"
            + self.selected_model
            + "_design_"
            + timestamp
            + ".csv"
        ...
        tk.messagebox.showinfo(
            "Information", "Design file " + self.design_csv_name + " has been created."
        )
    ...
    def update_defaults_button(self):
        ...
        tk.messagebox.showinfo(
            "Defaults Updated", "Defaults have been successfully updated."
        )
    ...
    def run_experiment(self, *args):
        ...
        save_path = filedialog.asksaveasfilename(
            initialfile=self.selected_model + "_datafarming_experiment_" + timestamp
        )
...
class Solver_Datafarming_Window(tk.Toplevel):
    def __init__(self, master):
        ...
        self.solver_select_label = tk.Label(
            master=self.solver_select_frame,
            text="Select Solver:",
            width=20,
            font="Calibir 13",
        )
        ...
        self.load_design_label = tk.Label(
            master=self.solver_select_frame, text=" OR ", width=20, font="Calibir 13"
        )
        self.load_design_label.grid(row=0, column=2)
        self.load_design_button = tk.Button(
            master=self.solver_select_frame,
            text="Load Solver Design",
            font="Calibir 11",
            width=20,
            command=self.load_solver_design,
        )
    ...
    def load_solver_design(self):
        ...
        self.clear_frame(
            frame=self.problem_model_factors_frame
        )  # Clear problem and model factor selections
        self.clear_frame(
            frame=self.problem_select_frame
        )  # Clear problem selection widgets
        ...
        for factor in self.factor_status:
            if self.factor_status[factor] is False:
        ...
        self.solver_name_label = tk.Label(
            master=self.solver_select_frame,
            text="Selected Solver: " + self.solver_name,
            font="Calibri 13",
            width=40,
        )
        ...
        for factor in self.default_factors:
            self.factor_datatype = self.solver_object.specifications[factor].get(
                "datatype"
            )
            self.factor_description = self.solver_object.specifications[factor].get(
                "description"
            )
            ...
            self.factorname_label.grid(
                row=self.factor_que_length, column=0, sticky=tk.N + tk.W
            )
    ...
    def show_solver_factors(self, *args):
        ...
        for factor in self.solver_object.specifications:
            ...
            factor_description = self.solver_object.specifications[factor].get(
                "description"
            )
            self.factor_default = self.solver_object.specifications[factor].get(
                "default"
            )
            ...
            if self.factor_datatype == float:
                ...
                self.default_value = (
                    tk.StringVar()
                )  # Variable to store selected bool factor state
                self.default_value.set("TRUE")  # Set default bool option
                self.bool_menu = ttk.OptionMenu(
                    self.solver_frame, self.default_value, "TRUE", "TRUE", "FALSE"
                )
                self.bool_menu.grid(
                    row=self.factor_que_length, column=2, sticky=tk.N + tk.W
                )
    ...
    def show_design_options(self):
        ...
        self.stack_menu = ttk.OptionMenu(
            ...
            stack_display,
            *self.stack_list,
            command=self.enable_create_design_button,
        )
        self.stack_menu.grid(row=1, column=1)
        self.stack_menu.configure(state="disabled")
        ...
        # Create design button
        if self.loaded_design is False:
            self.create_design_button = tk.Button(
                master=self.design_frame,
                text="Create Design",
                font="Calibri 13",
        ...
        if self.loaded_design is True:
            ...
            self.continue_design_button = tk.Button(
                master=self.design_frame,
                text="Continue w/o Changes",
                font="Calibri 13",
    ...
    def mod_design(self):
        ...
        self.csv_filename = (
            f"./data_farming_experiments/{self.design_filename}_design.csv"
        )
    ...
    def select_problem(self):
        ...
        self.problem_select_label = tk.Label(
            master=self.problem_select_frame,
            text="Select Problem:",
            width=20,
            font="Calibir 13",
        )
    ...
    def enable_stacks(self, *args):
        if self.loaded_design is False:  # cannot change stacks for loaded design
            self.stack_menu.configure(state="normal")
    ...
    def create_design(self, *args):
        ...
        with open(
            f"./data_farming_experiments/{self.design_filename}.txt", "w"
        ) as self.solver_design_factors:
            self.solver_design_factors.write("")

        if (
            self.loaded_design is False
        ):  # get factor settings for created design (not loaded)
            # List used for parsing design file
            self.factor_headers = []
            ...
            self.check_values = [
                self.checkstate.get() for self.checkstate in self.checkstate_list
            ]  # checkstate of each factor
            self.default_values = [
                self.default_value.get()
                for self.default_value in self.default_values_list
            ]  # default value of each factor
            self.min_values = [
                self.min_val.get() for self.min_val in self.min_list
            ]  # max value of each int & float factor
            self.max_values = [
                self.max_val.get() for self.max_val in self.max_list
            ]  # min value of each int & float factor
            self.dec_values = [
                self.dec_val.get() for self.dec_val in self.dec_list
            ]  # dec value of each float factor
            ...
            for factor in self.solver_object.specifications:
                ...
                if self.factor_include is True:
                    ...
                    if self.factor_datatype == float or self.factor_datatype == int:
                        ...
                        with open(
                            f"./data_farming_experiments/{self.design_filename}.txt",
                            "a",
                        ) as self.solver_design_factors:
                        ...
                        factor_options = [
                            True,
                            False,
                        ]  # list of values factor can take, temp hard coded for true/false
                        self.cross_design_factors[factor] = (
                            factor_options  # add factor to cross design dictionary
                        )
                        display = ""
                        for opt in factor_options:
                            opt_str = str(opt)
                            display += opt_str + "/"
                        self.solver_fixed_str[factor] = display[
                            :-1
                        ]  # add cross design factor to fixed str for design table
                        self.def_factor_names.append(
                            factor + " (cross)"
                        )  # list of factor names in cross design
            ...
            # Create design csv file with headers
            self.all_factor_headers = (
                self.factor_names + self.def_factor_names
            )  # combine factor names
            ...
            self.experiment = DataFarmingMetaExperiment(
                solver_name=self.solver_name,
    ...
    def show_problem_factors(self, *args):
        ...
        # Clear previous selections
        self.clear_frame(
            self.problem_model_factors_frame
        )  # clear problem-model factor selections
        ...
        self.headername_label = tk.Label(
            master=self.problem_model_factors_frame,
        ...
        self.problem_name = (
            self.problem_object.name
        )  # name of problem used for save files
        ...
        )  # name of model that relates to problem
        ...
        for factor in self.problem_object.specifications:
            self.factor_datatype = self.problem_object.specifications[factor].get(
                "datatype"
            )
            self.factor_description = self.problem_object.specifications[factor].get(
                "description"
            )
            self.factor_default = self.problem_object.specifications[factor].get(
                "default"
            )
        ...
        for factor in self.model_object.specifications:
            ...
            if self.factor_datatype == float:
                ...
                self.factorname_label.grid(
                    row=self.factor_que_length, column=3, sticky=tk.N + tk.W
                )
        ...
        # Save pair name
        self.pair_name_label = tk.Label(
            master=self.create_pair_frame,
            text="Save pair as:",
            width=20,
            font="Calibri 13",
        )
    ...
    def add_pair(self):
        ...
        if len(self.experiment_pairs) == 0:  # for first added pair create header
            self.pair_name_header = tk.Label(
                master=self.experiment_frame,
                text="Generated Pairs",
                width=20,
                font="Calibri 13 bold",
            )
            self.pair_name_header.grid(row=0, column=1)

            self.mac_rep_label = tk.Label(
                master=self.experiment_frame,
                text="# Macro Replications",
                font="Calibri 13 bold",
                width=20,
            )
            ...
            self.crn_macro_label = tk.Label(
                master=self.experiment_frame,
                text="CRN Across Macro?",
                font="Calibri 13 bold",
                width=20,
            )
            self.crn_macro_label.grid(row=0, column=6)

            self.norm_rep_label = tk.Label(
                master=self.experiment_frame,
                text="# Normalization Replications",
                font="Calibri 13 bold",
                width=0,
            )
            ...
            self.crn_macro_label = tk.Label(
                master=self.experiment_frame,
                text="CRN Across Initial Option?",
                font="Calibri 13 bold",
                width=25,
            )
            self.crn_macro_label.grid(row=0, column=9)

        self.problem_default_values = [
            self.default_value.get()
            for self.default_value in self.problem_default_values_list
        ]
        self.model_default_values = [
            self.default_value.get()
            for self.default_value in self.model_default_values_list
        ]
        ...
        for factor in self.problem_object.specifications:
            ...
            if self.factor_datatype == list:
                self.problem_fixed_factors[factor] = ast.literal_eval(
                    self.factor_default
                )
            ...
            if self.factor_datatype == tuple:
                ...
                if last_val != ",":
                    self.problem_fixed_factors[factor] = tuple(
        ...
        self.experiment_pairs[pair_name] = [
            self.experiment,
            self.problem_object.name,
            self.problem_fixed_factors,
            self.model_fixed_factors,
        ]  # Add experiment to dictionary by pair name
    ...
    def show_experiment_options(self, pair_name):
        ...
        # Pair name label
        self.pair_name_label = tk.Label(
            master=self.experiment_frame, text=pair_name, font="Calibri 11", width=20
        )
        ...
        self.mac_rep_var = (
            tk.StringVar()
        )  # variable to store number of macro replication of experiment
        self.macro_rep_vars[pair_name] = (
            self.mac_rep_var
        )  # store to macro list for all experiments

        self.mac_rep_entry = tk.Entry(
            master=self.experiment_frame,
            width=10,
            textvariable=self.mac_rep_var,
            justify="right",
        )
        self.mac_rep_entry.grid(row=current_row, column=2)

        # Run experiment button
        self.run_exp_button = tk.Button(
            master=self.experiment_frame,
            width=10,
            text="Run",
            font="Calibri 11",
            command=lambda: self.run_experiment(pair_name=pair_name),
        )
        self.run_exp_button.grid(row=current_row, column=3)
        self.run_buttons[pair_name] = (
            self.run_exp_button
        )  # store to run button list for all experiments

        # Number post replications
        self.post_rep_var = (
            tk.StringVar()
        )  # variable to store number of post replications of experiment
        self.post_rep_vars[pair_name] = (
            self.post_rep_var
        )  # store to post rep list for all experiments
        ...
        crn_budget_var = (
            tk.StringVar()
        )  # variable to hold true/false state of crn across budget
        self.crn_budget_menu = ttk.OptionMenu(
            self.experiment_frame, crn_budget_var, "True", *["True", "False"]
        )
        ...
        crn_macro_var = (
            tk.StringVar()
        )  # variable to hold true/false state of crn across macro reps
        self.crn_macro_menu = ttk.OptionMenu(
            self.experiment_frame, crn_macro_var, "False", *["True", "False"]
        )
        ...
        self.post_button = tk.Button(
            ...
            text="Post-Process",
            font="Calibri 11",
            command=lambda: self.post_replicate(
                pair_name=pair_name,
        ...
        self.post_buttons[pair_name] = (
            self.post_button
        )  # store to post button list for all experiments
        ...
        self.norm_rep_var = (
            tk.StringVar()
        )  # variable to store number of normalization replications of experiment
        self.norm_rep_vars[pair_name] = (
            self.norm_rep_var
        )  # store to norm rep list for all experiments

        self.norm_rep_entry = tk.Entry(
            master=self.experiment_frame,
            width=10,
            textvariable=self.norm_rep_var,
            justify="right",
        )
        ...
        crn_init_var = (
            tk.StringVar()
        )  # variable to hold true/false state of crn across initial option
        self.crn_init_menu = ttk.OptionMenu(
            self.experiment_frame, crn_init_var, "True", *["True", "False"]
        )
        ...
        self.norm_button = tk.Button(
            ...
            text="Normalize",
            font="Calibri 11",
            command=lambda: self.post_normalize(
                pair_name=pair_name, crn_across_init=crn_init_var.get()
            ),
        )
        self.norm_button.grid(row=current_row, column=10)
        self.norm_buttons[pair_name] = (
            self.norm_button
        )  # store to norm button list for all experiments
        ...
        self.save_button = tk.Button(
            ...
            text="Save",
            font="Calibri 11",
            command=lambda: self.save_results(pair_name=pair_name),
        )

        self.save_button.grid(row=current_row, column=11)
        self.save_buttons[pair_name] = (
            self.save_button
        )  # store to save button list for all experiments
        self.save_button.configure(state="disabled")

    def run_experiment(self, pair_name):
        current_exp = self.experiment_pairs[pair_name][
            0
        ]  # experiment is first element in stored list
    ...
    def save_results(self, pair_name):
        ...
        export_csv_filename = filedialog.asksaveasfilename(
            initialfile=pair_name + "_datafarming_experiment"
        )

        # Compute the performance metrics at each design point and print to csv.
        current_exp.report_statistics(
            solve_tols=[0.05, 0.10, 0.20, 0.50], csv_filename=export_csv_filename
        )
    ...
    def __init__(self, master, main_widow, forced_creation=False):
        if not forced_creation:
            ...
            self.crossdesign_problem_label = tk.Label(
                master=self.master, text="Select Problems:", font="Calibri 13"
            )
            self.crossdesign_problem_label.place(x=190, y=55)

            self.crossdesign_solver_label = tk.Label(
                master=self.master, text="Select Solvers:", font="Calibri 13"
            )
...
class Post_Processing_Window:
    ...
    def post_processing_run_function(self):
        ...
        if (
            self.n_postreps_entry.get().isnumeric() is not False
            and self.crn_across_budget_var.get() in self.crn_across_budget_list
            and self.crn_across_macroreps_var.get() in self.crn_across_macroreps_list
            and (
                self.meta is True
                and self.n_norm_postreps_entry.get().isnumeric()
                or self.meta is False
            )
        ):
            ...
            self.my_experiment.post_replicate(
                self.n_postreps, self.crn_across_budget, self.crn_across_macroreps
            )
        ...
        elif self.n_postreps_entry.get().isnumeric() is False:
...
class Post_Normal_Window:
    ...
    def __init__(self, master, experiment_list, main_window, meta=False):
        ...
        self.title = tk.Label(
            master=self.master, text=top_lab, font="Calibri 15 bold", justify="center"
        )
        ...
        self.n_proxy_sol_entry = ttk.Entry(
            master=self.master, textvariable=self.proxy_sol, justify=tk.LEFT, width=8
        )
        self.n_proxy_val_entry = ttk.Entry(
            master=self.master, textvariable=self.proxy_var, justify=tk.LEFT, width=8
        )
        self.n_initial_entry = ttk.Entry(
            master=self.master, textvariable=self.init_var, justify=tk.LEFT, width=10
        )
    ...
    def post_norm_run_function(self):
        ...
        # if self.n_postreps_entry.get().isnumeric() != False and self.n_postreps_init_opt_entry.get().isnumeric() != False and self.crn_across_budget_var.get() in self.crn_across_budget_list and self.crn_across_macroreps_var.get() in self.crn_across_macroreps_list:
        if self.n_postreps_init_opt_entry.get().isnumeric() is not False:
        ...
        elif self.n_postreps_init_opt_entry.get().isnumeric() is False:
            message = "Please enter a valid value for the number of postreplications at each recommended solution."
            tk.messagebox.showerror(title="Error Window", message=message)
...
class Plot_Window:
    ...
    def __init__(self, master, main_window, experiment_list=None, metaList=None):
        ...
        self.problem_menu = Listbox(
            self.master, selectmode=MULTIPLE, exportselection=False, width=10, height=6
        )
        ...
        self.settings_canvas.create_window(
            (0, 0), window=self.settings_frame, anchor="nw", tags="self.queue_frame"
        )
    ...
    def add_plot(self):
        ...
        if len(self.plot_exp_list) == 0 or str(plotType) == "Plot":
            ...
            self.bad_label = tk.Label(
                master=self.master, text=txt, font="Calibri 12", justify="center"
            )
        ...
        if self.plot_type_list[-1] == "Mean Progress Curve":
            path_name = plot_progress_curves(
                ...
                plot_CIs=bool(param_value_list[0]),
        ...
        for i, new_plot in enumerate(path_name):
            ...
            self.solver_button_added = tk.Label(
                master=self.tab_one, text=prob_text, font="Calibri 12", justify="center"
            )
            ...
            self.plot_type_button_added = tk.Label(
                master=self.tab_one, text=plotType, font="Calibri 12", justify="center"
            )
            ...
            self.plot_path = tk.Label(
                master=self.tab_one, text=new_plot, font="Calibri 12", justify="center"
            )
    ...
    def get_parameters_and_settings(self, a, plot_choice):
        ...
        if (
            ...
            entry = tk.Checkbutton(
                self.CI_canvas, variable=self.params[i], onvalue=True, offvalue=False
            )
            entry.select()
            # Creates the Check Mark that checks whether the plots will be plot together
            label = tk.Label(
        ...
        for param, param_val in param_list.items():
            if param == "normalize":
                ...
                label = tk.Label(
                    master=self.CI_canvas, text="Select Solver", font="Calibri 13"
                )
...
def problem_solver_abbreviated_name_to_unabbreviated(
    problem_or_solver, abbreviated_dictionary, unabbreviated_dictionary
):
