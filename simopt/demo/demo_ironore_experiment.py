"""
This script runs five solvers on 20 versions of the continous Iron Ore problem.
    ...
    for sd in st_devs:
        for hc in holding_costs:
            for inv in inven_stops:
                ...
                new_experiment.problem.name = f"IRONORECONT-1 with $\sigma={sd}$ and hc={hc} and is={inv}"
...
for i in range(n_problems):
    plot_progress_curves([experiments[solver_idx][i] for solver_idx in range(n_solvers)], plot_type="mean", all_in_one=True, plot_CIs=True, print_max_hw=True, normalize=True)
