...
class Solver(ABC):
    ...
    def evaluate(self, x: Solution | tuple, problem: Problem, n_reps: int = 1) -> Solution:
        ...
        start_n_reps = x.n_reps
        self.budget.request(n_reps)
        problem.simulate(x, n_reps)
        objective_factors = -np.array(problem.minmax)
        if not np.all(objective_factors == 1):
            for idx in range(start_n_reps, x.n_reps):
                x._objectives[idx] = objective_factors * x._objectives[idx]
                x._objectives_gradients[idx] = (
                    objective_factors[:, np.newaxis] * x._objectives_gradients[idx]
                )
