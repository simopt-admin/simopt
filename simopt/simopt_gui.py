...
class Experiment_Window(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        self.macro_definition = tk.Label(
            master=self.master, text="", font="Calibri 13"
        )
        ...
        for heading in self.heading_list:
            ...
            label = tk.Label(
                master=self.tab_one, text=heading, font="Calibri 14 bold"
            )
            label.grid(
        ...
        for heading in self.heading_list:
            ...
            label = tk.Label(
                master=self.tab_two, text=heading, font="Calibri 14 bold"
            )
            label.grid(
                row=0, column=self.heading_list.index(heading), padx=10, pady=3
            )
    ...
    def add_experiment(self, *args):
        ...
        if (
            ...
            and self.macro_entry.get().isnumeric() != False
        ):
    ...
    def confirm_problem_factors(self):
        ...
        for problem_factor in self.problem_factors_list:
            ...
            if index > 0:
                ...
                if (
                    keys[index - 1] == "initial_solution"
                    and type(ast.literal_eval(problem_factor.get())) == int
                ):
    ...
    def load_pickle_file_function(self):
        ...
        if filename != "No file selected":
            ...
            if filetype in acceptable_types:
                experiment_pathname = filename[
                    filename.index("experiments/outputs/") :
                ]
...
class Data_Farming_Window:
    ...
    def load_design(self):
        ...
        with open(self.design_csv_name) as design_file:
        ...
        with open(
            ...
            design_file.write("")
        with open(self.design_csv_name) as design_csv:
    ...
    def onFrameConfigure_factor(self, *args):
        self.factor_canvas.configure(
            scrollregion=self.factor_canvas.bbox("all")
        )
    ...
    def display_design_tree(self):
        ...
        self.num_factors = len(self.all_factor_headers) - len(
            default_factor_names
        )
        ...
        self.style.configure(
            "Treeview", foreground="black", font=("Calibri", 13)
        )
    ...
    def create_design(self, *args):
        ...
        for factor in self.model_object.specifications:
            ...
            if self.factor_datatype == list:
                self.fixed_factors[factor] = ast.literal_eval(
                    self.factor_default
                )
        ...
        with open(
            self.design_csv_name, "w", newline=""
        ) as self.model_design_csv:
            writer = csv.writer(self.model_design_csv)
            writer.writerow(
                ["Design #"] + self.factor_names + self.def_factor_names
            )
        ...
        for dp_index in range(self.n_design_pts):
            ...
            for factor_idx in range(len(self.factor_headers)):
                current_factor_designs.append(
                    design_table[factor_idx][dp_index]
                )
            data_insert = current_factor_designs + default_list

            # Write design points to csv file
            with open(
                self.design_csv_name, "a", newline=""
            ) as self.model_design_csv:
                writer = csv.writer(self.model_design_csv)
        ...
        tk.messagebox.showinfo(
            "Information",
            "Design file " + self.design_csv_name + " has been created.",
        )
    ...
    def run_experiment(self, *args):
        ...
        save_path = filedialog.asksaveasfilename(
            initialfile=self.selected_model
            + "_datafarming_experiment_"
            + timestamp
        )
...
class Solver_Datafarming_Window(tk.Toplevel):
    ...
    def load_solver_design(self):
        # Clear previous selections
        self.clear_frame(
            frame=self.solver_frame
        )  # Clear previous solver selections
        ...
        for factor in self.default_factors:
            self.factor_datatype = self.solver_object.specifications[
                factor
            ].get("datatype")
            self.factor_description = self.solver_object.specifications[
                factor
            ].get("description")
            self.factor_default = self.default_factors[factor]

            self.solver_frame.grid_rowconfigure(
                self.factor_que_length, weight=1
            )
    ...
    def show_solver_factors(self, *args):
        ...
        )  # name of solver used for save files
    ...
    def enable_stacks(self, *args):
        ...
        ):  # cannot change stacks for loaded design
            self.stack_menu.configure(state="normal")
    ...
    def create_design(self, *args):
        ...
        if (
            ...
            self.check_values = [
                self.checkstate.get()
                for self.checkstate in self.checkstate_list
            ...
            for factor in self.solver_object.specifications:
                ...
                if self.factor_include == True:
                    ...
                    if (
                        self.factor_datatype == float
                        or self.factor_datatype == int
                    ):
                        # Add factor to list of design factors in order that will be varied
                        self.factor_headers.append(factor)
                        # Factor names in csv are unedited
                        ...
                        self.factor_min = str(
                            self.min_values[self.maxmin_index]
                        )
                        self.factor_max = str(
                            self.max_values[self.maxmin_index]
                        )
                        self.maxmin_index += 1

                        if self.factor_datatype == float:
                            self.factor_dec = str(
                                self.dec_values[self.dec_index]
                            )
                            self.dec_index += 1
    ...
    def show_problem_factors(self, *args):
        ...
        for factor in self.problem_object.specifications:
            self.factor_datatype = self.problem_object.specifications[
                factor
            ].get("datatype")
            self.factor_description = self.problem_object.specifications[
                factor
            ].get("description")
            self.factor_default = self.problem_object.specifications[
                factor
            ].get("default")

            self.problem_frame.grid_rowconfigure(
                self.factor_que_length, weight=1
            )
        ...
        for factor in self.model_object.specifications:
            ...
            if self.factor_datatype == float:
                ...
                self.model_frame.grid_rowconfigure(
                    self.factor_que_length, weight=1
                )
    ...
    def add_pair(self):
        ...
        if (
            len(self.experiment_pairs) == 0
        ):  # for first added pair create header
        ...
        for factor in self.model_object.specifications:
            ...
            if self.factor_datatype == list:
                self.model_fixed_factors[factor] = ast.literal_eval(
                    self.factor_default
                )

            if self.factor_datatype == tuple:
                tuple_str = tuple(self.factor_default[1:-1].split(","))
                self.model_fixed_factors[factor] = tuple(
                    float(s) for s in tuple_str
                )
    ...
    def run_experiment(self, pair_name):
        ...
        self.run_buttons[pair_name].configure(
            bg="dark gray"
    ...
    def post_replicate(self, pair_name, crn_across_budget, crn_across_macro):
        ...
        self.post_buttons[pair_name].configure(
            bg="dark gray"
        )  # change color of button
    ...
    def post_normalize(self, pair_name, crn_across_init):
        ...
        self.norm_buttons[pair_name].configure(
            bg="dark gray"
        )  # change color of button
    ...
    def save_results(self, pair_name):
        ...
        current_exp.report_statistics(
            ...
            csv_filename=export_csv_filename,
        )

        self.save_buttons[pair_name].configure(
            bg="dark gray"
        )  # change color of button
    ...
    def update_problem_list_compatability(self):
        ...
        # from experiments.inputs.all_factors.py:
        self.problem_list = (
            temp_problem_list  # list of problems used for option menu
        )
...
class Post_Processing_Window:
    ...
    def post_processing_run_function(self):
        ...
        if (
            self.n_postreps_entry.get().isnumeric() != False
    ...
    def get_parameters_and_settings(self, a, plot_choice):
        ...
        for param, param_val in param_list.items():
            if param == "normalize":
                ...
                label = tk.Label(
                    master=self.CI_canvas, text=param, font="Calibri 13"
                )
                label.grid(row=i, column=0, padx=10, pady=3)
                entry = ttk.Entry(
                    master=self.CI_canvas,
...
def problem_solver_unabbreviated_to_object(
    problem_or_solver, unabbreviated_dictionary
):
