function [minmax d m VarNature VarBds FnGradAvail NumConstraintGradAvail, StartingSol, budget, ObjBd, OptimalSol] = QueueGG1Structure(NumStartingSol, seed)
% Inputs:
...
budget = [100, 30000]; %[100; 1000; 3000; 10000; 30000];
ObjBd=NaN;
OptimalSol=NaN;
...
if NumStartingSol<0 || (NumStartingSol ~= round(NumStartingSol)) || (seed <= 0) || (round(seed) ~= seed),
    ...
    %OldStream = RandStream.setDefaultStream(MyStream);
    
    if NumStartingSol == 0,
        StartingSol = NaN;
    elseif NumStartingSol==1,
        StartingSol=rand()*(thHigh-thLow)+thLow;
    else
        StartingSol=rand(NumStartingSol,1)*(thHigh-thLow)+thLow;
